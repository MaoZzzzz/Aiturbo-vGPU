...
def mlfq_sequence(now_time) -> None:
    path = "/home/tank/maozz/experiment/experiment1/fcfs.txt"
...
def main():
    ...
    while (True):
        ...
        if now_time % 120 == 0 or is_arrive:
            for i in range(len(init.jobs)):
                if (str(init.jobs[i].id) == str(job_id_1) or str(init.jobs[i].id) == str(job_id_2)):
                    run_program.delete_job(init.jobs[i].id, False)
                    # time.sleep(60)
                    
            for i in range(len(init.jobs)):
                if (str(init.jobs[i].id) == str(job_id_1) or str(init.jobs[i].id) == str(job_id_2)):
                    init_job(init.jobs[i])
                    run_program.run_job(init.jobs[i].id)

        finish_id = finish_job(now_time)
        if finish_id != "-1":
            if str(job_id_1) == finish_id:
                job_id_1 = max(job_id_1, job_id_2) + 1
            if str(job_id_2) == finish_id:
                job_id_2 = max(job_id_1, job_id_2) + 1
