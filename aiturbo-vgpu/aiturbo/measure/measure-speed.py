...
def base_line():
	...
	while(True):
		stop = input("Please enter whether to stop (Yes/No): ")
		if stop == "Yes":
			for job in job_list:
...
def measure_speed():
	...
	for batch_size in ['40']:
		for num_ps in range(10, 22, 2):	# to save time, change to xrange(1, num_node, 2)
			for num_worker in range(10, 20):	# to save time, change to xrange(1, num_node-num_ps+1, 2)
				job_id += 1
				...
				image = 'tensorflow/mps'
				script = '/init.py'
				prog = '/tf/benchmarks/scripts/tf_cnn_benchmarks/tf_cnn_benchmarks.py'
				work_dir = '/tf/benchmarks/scripts/image_classification/data/'
				mount_dir_prefix = '/data/k8s-workdir/measurement/'
				...
				measure_job.set_training('100',batch_size,"",gpus='')
				...
				while(True):
					...
					if counter >= 5:
						stat = (model_name, "", batch_size, num_ps, num_worker, speed_list, ps_cpu_usage_list, worker_cpu_usage_list)
						stats.append(stat)
